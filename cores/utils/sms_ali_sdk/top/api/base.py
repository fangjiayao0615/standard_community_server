...
class RestApi(object):
    ...
    def getResponse(self, authrize=None, timeout=30):
        ...
        # 获取response结果
        #=======================================================================
        print(self.__domain, self.__port)
        sys_parameters = {
            P_FORMAT: 'json',
        ...
        print(self.__httpmethod, url, body, header)

        response = requests.post(self.__domain + url, data=body, headers=header)
        if response.status_code != 200:
            raise RequestException('invalid http status ' + str(response.status_code) + ',detail body:' + response.content)
